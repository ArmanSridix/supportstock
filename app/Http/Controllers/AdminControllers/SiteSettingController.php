...
// slugify method
    ...
    /**************************/
    public function shareThis(Request $request)
    {

        $title = array('pageTitle' => "shareThis");

        $result = array();        
        $result['commonContent'] = $this->Setting->commonContent();

        $share_this_details = DB::table('settings')->where('name','share_this_script')->first();
        $result['share_this_details'] = $share_this_details;

        // echo "<pre>";
        // print_r($result['share_this_details']);exit;

        return view("admin.settings.general.share_this", $title)->with('result', $result);
    }

    public function updateShareThis(Request $request)
    {

        $update = DB::table('settings')->where('name', '=', 'share_this_script')->update([
            'value' => htmlentities($request->share_this_script),
    ...
    public function socialLink(Request $request)
    {
        ...
        $title = array('pageTitle' => "Social Links");

        $result = array();        
        $result['commonContent'] = $this->Setting->commonContent();

        $link_details = DB::table('social_links')->where('social_links_id',1)->first();
        $result['link_details'] = $link_details;

        return view("admin.settings.general.social_link", $title)->with('result', $result);
    }

    public function updateSocialLink(Request $request)
    {

        $update = DB::table('social_links')
    ...
    public function reasonOfCancelOrder()
    {
        $title = array('pageTitle' => Lang::get("labels.ListingReasonOfCancelOrder"));
        $result = array();
        $orders_cancle_reasons = DB::table('orders_status_reasons')
        ->where('reason_type', '=', 1)
        ->paginate(15);
        $result['orders_cancle_reasons'] = $orders_cancle_reasons;
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.Orders.reasonofcancel", $title)->with('result', $result);
    }
    public function addReasonOfCancelOrder()
    {
        $title = array('pageTitle' => Lang::get("labels.ListingReasonOfCancelOrder"));
        
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.Orders.addreasonofcancel", $title)->with('result', $result);
    }
    public function insertReasonOfCancelOrder(Request $request)
    {
        
        $reason_id = DB::table('orders_status_reasons')->insertGetId([
            'reason_type' => 1,
            'reason_status' => 1,
        ...
        if(!isset($reason_id)){
            $message = Lang::get("labels.addreasonOfCancelMessageFailed");
            return redirect()->back()->withErrors([$message]);
        }
        $message = Lang::get("labels.addreasonOfCancelMessage");
        return redirect()->back()->withErrors([$message]);
    }
    public function editReasonOfCancelOrder(Request $request)
    {
        $title = array('pageTitle' => Lang::get("labels.ListingReasonOfCancelOrder"));
        $result = array();
        $orders_cancle_reason = DB::table('orders_status_reasons')->where('reason_id', $request->id)->first();
        $result['orders_cancle_reason'] = $orders_cancle_reason;
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.Orders.editreasonofcancel", $title)->with('result', $result);
    }
    public function updateReasonOfCancelOrder(Request $request)
    {
        $update = DB::table('orders_status_reasons')->where('reason_id', '=', $request->reason_id)->update([
            'reason_status' => $request->reason_status,
            'status_reason' => $request->status_reason,
        ]);
        
        $message = Lang::get("labels.editreasonOfCancelMessage");
        return redirect()->back()->withErrors([$message]);
    }
    
    public function deleteReasonOfCancelOrder(Request $request)
    {
        
        $delete = DB::table('orders_status_reasons')->where('reason_id', '=', $request->reason_id)->delete();
        $message = Lang::get("labels.deletereasonOfCancelMessage");
        return redirect()->back()->withErrors([$message]);
    }
///////////////////////////
    public function reasonOfReturnOrder()
    {
        $title = array('pageTitle' => Lang::get("labels.ListingReasonOfReturnOrder"));
        $result = array();
        $orders_cancle_reasons = DB::table('orders_status_reasons')
        ->where('reason_type', '=', 2)
        ->paginate(15);
        $result['orders_cancle_reasons'] = $orders_cancle_reasons;
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.Orders.reasonofreturn", $title)->with('result', $result);
    }
    public function addReasonOfReturnOrder()
    {
        $title = array('pageTitle' => Lang::get("labels.ListingReasonOfReturnOrder"));
        
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.Orders.addreasonofreturn", $title)->with('result', $result);
    }
    public function insertReasonOfReturnOrder(Request $request)
    {
        ...
        if(!isset($reason_id)){
            $message = Lang::get("labels.addreasonOfReturnMessageFailed");
            return redirect()->back()->withErrors([$message]);
        }
        $message = Lang::get("labels.addreasonOfReturnMessage");
        return redirect()->back()->withErrors([$message]);
    }
    public function editReasonOfReturnOrder(Request $request)
    {
        $title = array('pageTitle' => Lang::get("labels.ListingReasonOfReturnOrder"));
        $result = array();
        $orders_cancle_reason = DB::table('orders_status_reasons')->where('reason_id', $request->id)->first();
        $result['orders_cancle_reason'] = $orders_cancle_reason;
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.Orders.editreasonofreturn", $title)->with('result', $result);
    }
    public function updateReasonOfReturnOrder(Request $request)
    {
        $update = DB::table('orders_status_reasons')->where('reason_id', '=', $request->reason_id)->update([
            'reason_status' => $request->reason_status,
            'status_reason' => $request->status_reason,
        ...
        $message = Lang::get("labels.editreasonOfReturnMessage");
        return redirect()->back()->withErrors([$message]);
    }
    public function deleteReasonOfReturnOrder(Request $request)
    {
        
        $delete = DB::table('orders_status_reasons')->where('reason_id', '=', $request->reason_id)->delete();
        $message = Lang::get("labels.deletereasonOfReturnMessage");
        return redirect()->back()->withErrors([$message]);
    }
////////////////////////
