...
class AdminController extends Controller
{
	...
	public function dashboard(Request $request){
		...
		$variableProduct = DB::select(DB::raw("SELECT inventory_detail_new.*,products_description.products_name , SUM(CASE WHEN stock_type = 'in' THEN stock ELSE 0 END) AS instocksum,SUM(CASE WHEN stock_type = 'out' THEN stock ELSE 0 END) AS outstocksum,(SUM(CASE WHEN stock_type = 'in' THEN stock ELSE 0 END)-SUM(CASE WHEN stock_type = 'out' THEN stock ELSE 0 END)) as finalstock FROM `inventory` 
        LEFT JOIN products on products.products_id = inventory.products_id 
        LEFT JOIN inventory_detail_new on inventory.inventory_ref_id = inventory_detail_new.inventory_ref_id 
        LEFT JOIN products_description ON products_description.products_id = inventory_detail_new.products_id
        WHERE products.products_type = 1
        GROUP by attributes_ids DESC"));
		...
		//   $month_ini =date("Y-m-d", mktime(0, 0, 0, date("m"), 1))." 00:00:00";
		//   $month_end =  date("Y-m-d", mktime(0, 0, 0, date("m")+1, 0))." 23:59:59";
		

		//   $customers = $this->Customers->whereBetween('created_at', [$month_ini, $month_end])->get();
   ...
   public function profile(Request $request){
	...
	public function addrole(Request $request){
		...
		if(count($roles)>0){
			...
			$city_view   = $roles[0]->city_view;
			$city_create = $roles[0]->city_create;
			$city_update = $roles[0]->city_update;
			$city_delete = $roles[0]->city_delete;

			$pincode_view   = $roles[0]->pincode_view;
			$pincode_create = $roles[0]->pincode_create;
			$pincode_update = $roles[0]->pincode_update;
			$pincode_delete = $roles[0]->pincode_delete;

			$assured_logo_view   = $roles[0]->assured_logo_view;
			$assured_logo_update = $roles[0]->assured_logo_update;

			$corp_request_view   = $roles[0]->corp_request_view;
			$corp_request_update = $roles[0]->corp_request_update;
			$corp_request_delete = $roles[0]->corp_request_delete;

			$cms_view   = $roles[0]->cms_view;
			$cms_add = $roles[0]->cms_add;
			$cms_update = $roles[0]->cms_update;
			$cms_delete = $roles[0]->cms_delete;

			$cod_view   = $roles[0]->cod_view;
			$cod_update = $roles[0]->cod_update;

			$contact_comment_view   = $roles[0]->contact_comment_view;
			$contact_comment_update = $roles[0]->contact_comment_update;
			$contact_comment_delete	 = $roles[0]->contact_comment_delete;

			$product_enquery_view   = $roles[0]->product_enquery_view;
			$product_enquery_update = $roles[0]->product_enquery_update;
			$product_enquery_delete	 = $roles[0]->product_enquery_delete;

			$sell_on_ss_view   = $roles[0]->sell_on_ss_view;
			$sell_on_ss_update = $roles[0]->sell_on_ss_update;
			$sell_on_ss_delete	 = $roles[0]->sell_on_ss_delete;
			...
			$assured_logo_view   = '0';
			$assured_logo_update = '0';

			$corp_request_view   = '0';
			$corp_request_update = '0';
			$corp_request_delete = '0';
			...
			$contact_comment_view   = '0';
			$contact_comment_update = '0';
			$contact_comment_delete	 = '0';

			$product_enquery_view   = '0';
			$product_enquery_update = '0';
			$product_enquery_delete	 = '0';
		...
		$result2[22]['link_name'] = 'Assured Logo';
		$result2[22]['permissions'] = array(
					'0'=>array('name'=>'assured_logo_view','value'=>$assured_logo_view),
					'1'=>array('name'=>'assured_logo_update','value'=>$assured_logo_update)
					);

		$result2[23]['link_name'] = 'cityMaster';
		$result2[23]['permissions'] = array(
					'0'=>array('name'=>'city_view','value'=>$city_view),
					'1'=>array('name'=>'city_create','value'=>$city_create),
					'2'=>array('name'=>'city_update','value'=>$city_update),
					'3'=>array('name'=>'city_delete','value'=>$city_delete),
					);

		$result2[24]['link_name'] = 'pincodeMaster';
		$result2[24]['permissions'] = array(
					'0'=>array('name'=>'pincode_view','value'=>$pincode_view),
					'1'=>array('name'=>'pincode_create','value'=>$pincode_create),
					'2'=>array('name'=>'pincode_update','value'=>$pincode_update),
					'3'=>array('name'=>'pincode_delete','value'=>$pincode_delete),
					);

		$result2[25]['link_name'] = 'corporateRequest';
		$result2[25]['permissions'] = array(
					'0'=>array('name'=>'corp_request_view','value'=>$corp_request_view),
					'1'=>array('name'=>'corp_request_update','value'=>$corp_request_update),
					'2'=>array('name'=>'corp_request_delete','value'=>$corp_request_delete)
					);

		$result2[26]['link_name'] = 'cms';
		$result2[26]['permissions'] = array(
					'0'=>array('name'=>'cms_view','value'=>$cms_view),
					'1'=>array('name'=>'cms_add','value'=>$cms_add),
					'2'=>array('name'=>'cms_update','value'=>$cms_update),
					'3'=>array('name'=>'cms_delete','value'=>$cms_delete),
					);

		$result2[27]['link_name'] = 'COD';
		$result2[27]['permissions'] = array(
					'0'=>array('name'=>'cod_view','value'=>$cod_view),
					'1'=>array('name'=>'cod_update','value'=>$cod_update)
					);

		$result2[28]['link_name'] = 'contact_comment';
		$result2[28]['permissions'] = array(
					'0'=>array('name'=>'contact_comment_view','value'=>$contact_comment_view),
					'1'=>array('name'=>'contact_comment_update','value'=>$contact_comment_update),
					'2'=>array('name'=>'contact_comment_delete','value'=>$contact_comment_delete)
					);

		$result2[29]['link_name'] = 'product_enquery';
		$result2[29]['permissions'] = array(
					'0'=>array('name'=>'product_enquery_view','value'=>$product_enquery_view),
					'1'=>array('name'=>'product_enquery_update','value'=>$product_enquery_update),
					'2'=>array('name'=>'product_enquery_delete','value'=>$product_enquery_delete)
					);

		$result2[30]['link_name'] = 'sell_on_ss';
		$result2[30]['permissions'] = array(
					'0'=>array('name'=>'sell_on_ss_view','value'=>$sell_on_ss_view),
					'1'=>array('name'=>'sell_on_ss_update','value'=>$sell_on_ss_update),
					'2'=>array('name'=>'sell_on_ss_delete','value'=>$sell_on_ss_delete)
					);
	...
	public function addnewroles(Request $request){
		...
		$roles = DB::table('manage_role')->where('user_types_id',$request->user_types_id)->insert([
						...
						'city_view' => $request->city_view,
						'city_create' => $request->city_create,
						'city_update' => $request->city_update,
						'city_delete' => $request->city_delete,

						'pincode_view' => $request->pincode_view,
						'pincode_create' => $request->pincode_create,
						'pincode_update' => $request->pincode_update,
						'pincode_delete' => $request->pincode_delete,

						'assured_logo_view' => $request->assured_logo_view,
						'assured_logo_update' => $request->assured_logo_update,

						'corp_request_view' => $request->corp_request_view,
						'corp_request_update' => $request->corp_request_update,
						'corp_request_delete' => $request->corp_request_delete,

						'cms_view' => $request->cms_view,
						'cms_add' => $request->cms_add,
						'cms_update' => $request->cms_update,
						'cms_delete' => $request->cms_delete,

						'cod_view' => $request->cod_view,
						'cod_update' => $request->cod_update,

						'contact_comment_view' => $request->contact_comment_view,
						'contact_comment_update' => $request->contact_comment_update,
						'contact_comment_delete' => $request->contact_comment_delete,

						'product_enquery_view' => $request->product_enquery_view,
						'product_enquery_update' => $request->product_enquery_update,
						'product_enquery_delete' => $request->product_enquery_delete,

						'sell_on_ss_view' => $request->sell_on_ss_view,
						'sell_on_ss_update' => $request->sell_on_ss_update,
						'sell_on_ss_delete' => $request->sell_on_ss_delete,
  ...
  //editCategoriesRoles
	...
	public function editContactUs(){
      ...
      return view("admin/contact_us/edit")->with('contactDetails', $contactDetails)->with('result', $result);
    }
    /************** edit contact us ****************/

	/************** update contact us ****************/
	public function updateContactUs(Request $request){
		$contact_update = DB::table('contact_us')->where('contact_us_id','=',1)
			->update([
	...
	public function editAboutUs(){
      ...
      $termsDetails = DB::table('cms')->where('cms_id','=', 1)->first();

      $result['commonContent'] = $this->Setting->commonContent();

      return view("admin/about_us/edit")->with('termsDetails', $termsDetails)->with('result', $result);
    }
    /************** edit edit term of use ****************/

	/************** update edit term of use ****************/
	public function updateAboutUs(Request $request){
		$about_update = DB::table('cms')->where('cms_id','=',1)
			->update([
				'cms_text'	=>	$request->about_us_text
			]);
	...
	/************** edit Seller Benefits ****************/
	public function editSellerBenefits(){

		$seller_benefits = DB::table('cms')->where('cms_id','=', 4)->first();
  
		$result['commonContent'] = $this->Setting->commonContent();
  
		return view("admin/seller_benefits/edit")->with('seller_benefits', $seller_benefits)->with('result', $result);
	  }
	  /************** edit Seller Benefits ****************/
  
	  /************** update Seller Benefits ****************/
	  public function updateSellerBenefits(Request $request){
		  $about_update = DB::table('cms')->where('cms_id','=',4)
			  ->update([
				  'cms_text'	=>	$request->about_us_text
			  ]);
	  ...
	  /************** edit Corporate Login Benefit's ****************/
	public function editCorporateLoginBenefits(){

		$corporate_login_benefits = DB::table('cms')->where('cms_id','=', 5)->first();
  
		$result['commonContent'] = $this->Setting->commonContent();
  
		return view("admin/corporate_login_benefits/edit")->with('corporate_login_benefits', $corporate_login_benefits)->with('result', $result);
	  }
	  /************** edit Corporate Login Benefit's ****************/
  
	  /************** update Corporate Login Benefit's ****************/
	  public function updateCorporateLoginBenefits(Request $request){
		  $about_update = DB::table('cms')->where('cms_id','=',5)
			  ->update([
				  'cms_text'	=>	$request->about_us_text
			  ]);
	...
	public function editTermsAndConditions(){
		...
		$termsAndConditionsDetails = DB::table('cms')->where('cms_id','=', 2)->first();
  
		$result['commonContent'] = $this->Setting->commonContent();
  
		return view("admin/terms_and_conditions/edit")->with('termsAndConditionsDetails', $termsAndConditionsDetails)->with('result', $result);
	  }
	  /************** edit Terms And Conditions ****************/
  
	  /************** update Terms And Conditions ****************/
	  public function updateTermsAndConditions(Request $request){
		  $about_update = DB::table('cms')->where('cms_id','=',2)
			  ->update([
				  'cms_text'	=>	$request->terms_and_conditions_text
			  ]);
	...
	public function editPrivacyPolicy(){
		...
		$privacyPolicyDetails = DB::table('cms')->where('cms_id','=', 3)->first();
  
		$result['commonContent'] = $this->Setting->commonContent();
  
		return view("admin/privacy_policy/edit")->with('privacyPolicyDetails', $privacyPolicyDetails)->with('result', $result);
	  }
	  /************** edit PRIVACY POLICY ****************/
  
	  /************** update PRIVACY POLICY ****************/
	  public function updatePrivacyPolicy(Request $request){
		  $about_update = DB::table('cms')->where('cms_id','=',3)
			  ->update([
				  'cms_text'	=>	$request->privacy_policy_text
			  ]);
	...
	public function displayFaq(){
		...
		$result['commonContent'] = $this->Setting->commonContent();
		return view("admin/faq/index")->with('faqList', $faqList)->with('result', $result);
	  }
	  /************** display FAQ ****************/
	  ...
	  public function addFaq(){
		$result['commonContent'] = $this->Setting->commonContent();
		return view("admin/faq/add")->with('result', $result);
	  }
	  /************** add FAQ ****************/
	...
	public function editFaq(Request $request){
		// echo $request->faq_id;exit;
		$faqDetails = DB::table('faq')->where('faq_id','=',$request->faq_id)->first();

		$result['commonContent'] = $this->Setting->commonContent();
		return view("admin/faq/edit")->with('faqDetails', $faqDetails)->with('result', $result);
	  }
	  /************** edit FAQ ****************/
	  ...
	  public function updateFaq(Request $request){
		if($request->faq_text == ""){
		  ...
		  $faq_update = DB::table('faq')->where('faq_id','=',$request->faq_id)
			  ->update([
				  'faq_title'	=>	$request->faq_title,
				  'faq_text'	=>	$request->faq_text,
				  'faq_status'	=>	$request->faq_status
	  ...
	  public function faqDelete(Request $request){
		$faq_update = DB::table('faq')->where('faq_id','=',$request->faq_id)->delete();
	...
	public function contactComment(Request $request){
		$comment_list = DB::table('contact_us_comments')
            ->select('contact_us_comments.*')
            ->orderBy('contact_us_comments.contact_us_comments_id', 'DESC')
            ->paginate(15);

		$result['commonContent'] = $this->Setting->commonContent();
		return view("admin/contact_us/contact_us_comment")->with('comment_list', $comment_list)->with('result', $result);
	}

	public function contactCommentDelete(Request $request){
		$faq_update = DB::table('contact_us_comments')->where('contact_us_comments_id','=',$request->id)->delete();
	...
	public function productEnquiry(){
      	...
      	$productEnquiryList = DB::table('enquiry_products')
      		->leftJoin('products_description','products_description.products_id','=','enquiry_products.enProductId')
      		->leftJoin('cities','cities.cities_id','=','enquiry_products.enCity')
      		->leftJoin('pincodes','pincodes.pincodes_id','=','enquiry_products.enPincode')
			->select('enquiry_products.*','products_description.products_name','cities.cities_name','pincodes.pincodes_val')
      		->orderBy('enquiry_products.enquiry_products_id', 'DESC')
      		->paginate(15);

      	$result['commonContent'] = $this->Setting->commonContent();

      	return view("admin/products/product_enquiry")->with('productEnquiryList', $productEnquiryList)->with('result', $result);
    }
    /************** product enquiry ****************/

    /************** sell on supportstock ****************/
	public function sellOnSupportStock(){

      	$sellList = DB::table('sell_on_support')
			->select('sell_on_support.*')
      		->orderBy('sell_on_support.sell_on_support_id', 'DESC')
      		->paginate(10);

      	$result['commonContent'] = $this->Setting->commonContent();

      	return view("admin/sell_support")->with('sellList', $sellList)->with('result', $result);
    }
    /************** sell on supportstock ****************/

    /************** edit logo ****************/
	public function editAssuredLogo(Request $request){
		$images = new Images;
    	$allimage = $images->getimages();

		$logoDetails = DB::table('assured_logo')
			->LeftJoin('image_categories as iconTable', function ($join) {
                $join->on('iconTable.image_id', '=', 'assured_logo.assured_logo_image')
                    ->where(function ($query) {
            ...
            ->select('assured_logo.*','iconTable.path as iconpath')
			->where('assured_logo.assured_logo_id','=',1)
			->get();

		$result['commonContent'] = $this->Setting->commonContent();
		return view("admin/logo_assured")->with('logoDetails', $logoDetails)->with('result', $result)->with('allimage', $allimage);
	}
	/************** edit logo ****************/
  
	/************** update logo ****************/
	public function updateAssuredLogo(Request $request){
		...
		$faq_update = DB::table('assured_logo')->where('assured_logo_id','=',1)
			->update([
				  'assured_logo_image'	=>	$uploadIcon
			]);
	...
	/************** invoice logo ****************/
	public function invoiceLogo(Request $request){
		$images = new Images;
    	$allimage = $images->getimages();

		$logoDetails = DB::table('invoice_logo')
			->LeftJoin('image_categories as iconTable', function ($join) {
                $join->on('iconTable.image_id', '=', 'invoice_logo.invoice_logo_image')
                    ->where(function ($query) {
                        $query->where('iconTable.image_type', '=', 'THUMBNAIL')
                            ->where('iconTable.image_type', '!=', 'THUMBNAIL')
                            ->orWhere('iconTable.image_type', '=', 'ACTUAL');
            ...
            ->select('invoice_logo.*','iconTable.path as iconpath')
			->where('invoice_logo.invoice_logo_id','=',1)
			->get();

		$result['commonContent'] = $this->Setting->commonContent();
		return view("admin/logo_invoice")->with('logoDetails', $logoDetails)->with('result', $result)->with('allimage', $allimage);
	}
	...
	public function updateInvoiceLogo(Request $request){
		...
		$logo_update = DB::table('invoice_logo')->where('invoice_logo_id','=',1)
			->update([
				  'invoice_logo_image'	=>	$uploadIcon
			]);
	...
	public function editPayment(){
		...
		$paymentDetails = DB::table('cms')->where('cms_id','=', 6)->first();
  
		$result['commonContent'] = $this->Setting->commonContent();
  
		return view("admin/cms/payment")->with('paymentDetails', $paymentDetails)->with('result', $result);
	}
	
	public function updatePayment(Request $request){
		$about_update = DB::table('cms')->where('cms_id','=',6)
			->update([
				'cms_text'	=>	$request->cms_text
			]);
	...
	public function editShipping(){
		...
		$shippingDetails = DB::table('cms')->where('cms_id','=', 7)->first();
  
		$result['commonContent'] = $this->Setting->commonContent();
  
		return view("admin/cms/shipping")->with('shippingDetails', $shippingDetails)->with('result', $result);
	}
	
	public function updateShipping(Request $request){
		$update = DB::table('cms')->where('cms_id','=',7)
			->update([
				'cms_text'	=>	$request->cms_text
			]);
	...
	/************** edit Cancellation & Returns ****************/
	public function editCancelReturn(){

		$cancelDetails = DB::table('cms')->where('cms_id','=', 8)->first();
  
		$result['commonContent'] = $this->Setting->commonContent();
  
		return view("admin/cms/cancel_return")->with('cancelDetails', $cancelDetails)->with('result', $result);
	}
	
	public function updateCancelReturn(Request $request){
		$update = DB::table('cms')->where('cms_id','=',8)
			->update([
				'cms_text'	=>	$request->cms_text
			]);
	...
	public function editRefunds(){
		...
		$refundDetails = DB::table('cms')->where('cms_id','=', 9)->first();
  
		$result['commonContent'] = $this->Setting->commonContent();
  
		return view("admin/cms/refund")->with('refundDetails', $refundDetails)->with('result', $result);
	}
	
	public function updateRefunds(Request $request){
		$update = DB::table('cms')->where('cms_id','=',9)
			->update([
				'cms_text'	=>	$request->cms_text
			]);
	...
	public function editReturnPolicy(){
		...
		$returnDetails = DB::table('cms')->where('cms_id','=', 10)->first();
  
		$result['commonContent'] = $this->Setting->commonContent();
  
		return view("admin/cms/return_policy")->with('returnDetails', $returnDetails)->with('result', $result);
	}
	
	public function updateReturnPolicy(Request $request){
		$update = DB::table('cms')->where('cms_id','=',10)
			->update([
				'cms_text'	=>	$request->cms_text
			]);
	...
	public function editSecurity(){
		...
		$securityDetails = DB::table('cms')->where('cms_id','=', 11)->first();
  
		$result['commonContent'] = $this->Setting->commonContent();
  
		return view("admin/cms/security")->with('securityDetails', $securityDetails)->with('result', $result);
	}
	
	public function updateSecurity(Request $request){
		$update = DB::table('cms')->where('cms_id','=',11)
			->update([
				'cms_text'	=>	$request->cms_text
			]);
	...
	public function editCustomerService(){
		...
		$serviceDetails = DB::table('cms')->where('cms_id','=', 12)->first();
  
		$result['commonContent'] = $this->Setting->commonContent();
  
		return view("admin/cms/customer_service")->with('serviceDetails', $serviceDetails)->with('result', $result);
	}
	
	public function updateCustomerService(Request $request){
		$update = DB::table('cms')->where('cms_id','=',12)
			->update([
				'cms_text'	=>	$request->cms_text
			]);
	...
	public function editCopyright(){
		...
		$copyrightDetails = DB::table('cms')->where('cms_id','=', 13)->first();
  
		$result['commonContent'] = $this->Setting->commonContent();
  
		return view("admin/cms/copyright")->with('copyrightDetails', $copyrightDetails)->with('result', $result);
	}
	
	public function updateCopyright(Request $request){
		$update = DB::table('cms')->where('cms_id','=',13)
			->update([
				'cms_text'	=>	$request->cms_text
			]);
