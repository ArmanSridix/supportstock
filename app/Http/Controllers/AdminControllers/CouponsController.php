...
class CouponsController extends Controller
{
    ...
    public function insert(Request $request)
    {
        ...
        if ($request->used_by !== null) {
            ...
            // $used_by = implode(',', $request->used_by);
        } else {
            $used_by = '';
        }
        ...
        if ($validator->fails()) {
            ...
            if (count($couponInfo) > 1) {
                ...
                $coupon_id = $this->Coupon->couponupdate($coupans_id, $code, $description, $discount_type, $amount, $individual_use,
                    ...
                    $limit_usage_to_x_items, $product_categories, $excluded_product_categories,
    ...
    public function newUserCoupon(Request $request)
    {
        ...
        $title = array('pageTitle' => "New user coupon");
        $result = array();
        $message = array();
        $result['message'] = $message;
        //coupon
        ...
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.coupons.new_user_coupon", $title)->with('result', $result);
    }

    public function updateUserCoupon(Request $request)
    {

        DB::table('coupons')->where('coupans_id', '=', 7)->update(
            [
                'code' => $request->code,
                'description' => $request->description,
                'amount' => $request->amount
