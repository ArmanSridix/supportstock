...
class AlertController extends Controller
{
	...
	public function createOtpAlert($existUser){	

		if(!empty($existUser['email'])){
			try {
					
				Mail::send('/mail/createOtp', ['userData' => $existUser], function($m) use ($existUser){
					$m->to($existUser['email'])->subject("Supportstock Sign up OTP")->getSwiftMessage()
					->getHeaders()
	...
	public function fPasswordOtpAlert($userExist){	

		if(!empty($userExist[0]->email)){
			try {
					
				Mail::send('/mail/fPasswordOtp', ['userData' => $userExist], function($m) use ($userExist){
					$m->to($userExist[0]->email)->subject("Supportstock forget password OTP")->getSwiftMessage()
					->getHeaders()
					->addTextHeader('x-mailgun-native-send', 'true');
				});
	...
	public function contactusOtpAlert($data){	

		if(!empty($data['contact_mail'])){
			try {
					
				Mail::send('/mail/contactUsOtp', ['data' => $data], function($m) use ($data){
					$m->to($data['contact_mail'])->subject("Supportstock OTP")->getSwiftMessage()
					->getHeaders()
					->addTextHeader('x-mailgun-native-send', 'true');
				});
	...
	public function orderAlert($ordersData){
		...
		//if($alertSetting[0]->order_email==1){
			...
			// 		$m->to($ordersData['orders_data'][0]->admin_email)->subject("An order has been placed")->getSwiftMessage()
			// 		->getHeaders()
			// 		->addTextHeader('x-mailgun-native-send', 'true');
			// 	});
			// 	} catch (\Exception $e) {
			...
			if(!empty($ordersData['orders_data'][0]->email)){
				try {
					...
					Mail::send('/mail/orderEmail', ['ordersData' => $ordersData], function($m) use ($ordersData){
						$m->to($ordersData['orders_data'][0]->email)->subject("An order has been placed")->getSwiftMessage()
						->getHeaders()
						->addTextHeader('x-mailgun-native-send', 'true');
					});

					// Mail::send('/mail/orderEmail', ['ordersData' => $ordersData], function ($message)use ($ordersData) {
					//     $message->from('aryabhata16@gmail.com', 'Attukari');

					//     $message->to($ordersData['orders_data'][0]->email)->subject("An order has been placed");
					// });
	...
	public function orderStatusAlert($data){	

		if(!empty($data['email'])){
			try {
					
				Mail::send('/mail/orderStatusMail', ['data' => $data], function($m) use ($data){
					$m->to($data['email'])->subject("Supportstock Order Status")->getSwiftMessage()
					->getHeaders()
					->addTextHeader('x-mailgun-native-send', 'true');
				});
