...
class OrdersController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        if(Session::has('supportUserDetails')){
            ...
            $slot_list = DB::table('delivery_slots')->get();
            $result['slot_list'] = $slot_list;
            // echo "<pre>";
            // print_r($result);exit;
            $cod_limit_details = array();
            if ($current_user_details['user_type']=='Corporate') {
                $cod_limit_details = DB::table('cash_on_delivery_limit')
                    ->where('cod_user_type','=', 'Corporate')
                    ->first();
            }else{
                $cod_limit_details = DB::table('cash_on_delivery_limit')
                    ->where('cod_user_type','=', 'Normal')
                    ->first();
            }
            $result['cod_limit_details'] = $cod_limit_details;
            ...
            // echo "<pre>";
            // print_r($result['paymentMethods']);exit;
            $dBankTransfer = $this->order->payments_setting_for_directbank();
            $dBankTransferInformation = array(
                'account_name' => $dBankTransfer['account_name']->value,
                'account_number' => $dBankTransfer['account_number']->value,
                'payment_method' => $dBankTransfer['account_name']->payment_method,
                'bank_name' => $dBankTransfer['bank_name']->value,
                'short_code' => $dBankTransfer['short_code']->value,
                'iban' => $dBankTransfer['iban']->value,
                'swift' => $dBankTransfer['swift']->value,
            );
            $result['dBankTransferInformation'] = $dBankTransferInformation;

            // echo "<pre>";
            // print_r($result['dBankTransferInformation']);exit;
            
            return view("web.checkout")->with(['result' => $result])->with('flate_rate', $flate_rate);
        }else{
    ...
    public function cashOnDeliveryOrder(Request $request)
    {
        ...
        if(Session::has('supportUserDetails')){
            ...
            $input['customers_mail'] = $current_user_details['email'];
            $input['firstname'] = $current_user_details['first_name'];
            $input['lastname'] = $current_user_details['last_name'];
            $input['delivery_phone'] = $current_user_details['phone'];
            $input['order_comments'] = '';
            $input['paymentResponseData'] = '';

            $cart_items = $this->cart->myCart(array());
            ...
            if (count($cart_items) > 0) {
                foreach ($cart_items as $products) {
                    ...
                    if ($products->customers_basket_quantity > $check['stock']) {
                        ...
                        $returnData['out_of_stock_product_id'] = $products->products_id;
                        $returnData['out_of_stock_product'] = $products;
                        //return response()->json(['returnData'=>$returnData]);

                        $message = $products->products_name." is out of stock.";
                        return redirect()->back()->with('outStockMessage', $message);
                    }
                }
            }else{
            ...
            if ($placeOrder['ordersStatus'] == 'success') {
                ...
                return redirect()->back()->with('outStockMessage', $returnData['massage']);
            }
    ...
    public function onlineOrderPayment(Request $request)
    {
        $input = array();
        if(Session::has('supportUserDetails')){
            ...
            $input = $request->all();
            $input['payment_method'] = 'razor_pay';
            $input['customers_id'] = $customers_id;
            $input['customers_mail'] = $current_user_details['email'];
            $input['firstname'] = $current_user_details['first_name'];
            $input['lastname'] = $current_user_details['last_name'];
            ...
            $input['order_comments'] = '';
            $input['paymentResponseData'] = '';
            $input['logo_img'] = asset('frontEnd/images/logo.png');
            ...
            // echo "<pre>";
            // print_r($payments_setting);exit;

            return view("web.payWithRazorpay", ['input' => $input, 'payments_setting' => $payments_setting])->with(['result' => $result]);
        }else{
           return redirect()->back()->with('middleWareloginError', "Not login");
        }
    ...
    public function onlinePaymentReturn(Request $request){
        $returnData = array();
        $payments_setting = $this->order->payments_setting_for_razorpay();
        ...
        if ($success === true)
        {
            $placeOrder = $this->order->place_order($_POST);

            // $html = "<p>Your payment was successful</p>
            //          <p>Payment ID: {$_POST['razorpay_payment_id']}</p>";
            $returnData['order_status'] = 'success';
            $returnData['message'] = 'Order complete';
            $returnData['payment_id'] = $_POST['razorpay_payment_id'];
        ...
        else
        {
            ...
            //          <p>{$error}</p>";
            $returnData['order_status'] = 'fail';
            $returnData['message'] = 'Your payment failed';
        }

        $result['commonContent'] = $this->index->commonContent();

        return view("web.paymentSuccess", ['returnData' => $returnData])->with(['result' => $result]);
    }
    /*************** online order payment *******************/
    ...
    public function orders(Request $request)
    {
        ...
        if(Session::has('supportUserDetails')){
            ...
            $result1 = $this->order->orderList($customers_id);
           
            $result['orders_status_reasons'] = DB::table('orders_status_reasons')->where('reason_status','=',1)->get();
            if (!empty($result1['orders'])) {
                $result['orderList'] = $result1['orders'];
                // echo "<pre>";
                // print_r($result['orderList']);exit;
                return view("web.orders")->with(['result' => $result]);
            }else{
    ...
    public function updateOrderProductStatus(Request $request)
    {
        ...
        DB::table('orders_products')
            ->where('orders_products_id', $request->orders_products_id)
            ->update([
                'cancel_return_status' => $request->cancel_return_status,
                'cancel_return_reason' => $request->cancel_return_reason
            ]);

        /********************/
        $orderProductDetails = DB::table('orders_products')->where('orders_products_id', '=', $request->orders_products_id)->get();
        if (!empty($orderProductDetails)) {
            $orders_id = $orderProductDetails[0]->orders_id;
            $final_price = $orderProductDetails[0]->final_price;

            $orderDetail = DB::table('orders')->where('orders_id', $orders_id)->first();
            $cancelReturnPrice = $orderDetail->cancelReturnPrice;
            $cancelReturnPrice = $cancelReturnPrice+$final_price;

            DB::table('orders')->where('orders_id', $orders_id)->update(
            [
                'cancelReturnPrice' => $cancelReturnPrice
            ]);
        }
        /********************/

        if ($request->cancel_return_status==1) {
            $orders_products = DB::table('orders_products')->where('orders_products_id', '=', $request->orders_products_id)->get();
                ...
                if ($product_detail->products_type == 1) {
                    ...
                    foreach ($product_attribute as $attribute) {
                        ...
                        DB::table('inventory_detail')->insert([
                            ...
                            DB::table('inventory_detail_new')->insert([
                                'inventory_ref_id' => $inventory_ref_id,
                                'products_id' => $products_data->products_id,
                                'attributes_ids' =>  $prodocuts_attributes->products_attributes_id,
                            ]);
    ...
    public function orderInvoice(Request $request, $id)
    {
        ...
        if(Session::has('supportUserDetails')){
            ...
            // echo "<pre>";
            // print_r($result['orders'][0]);exit;

            if ($result['res'] = "view-order") {
                return view("web.order_invoice")->with(['result' => $result]);
            } else {
                return redirect('orders');
            }
        }else{
