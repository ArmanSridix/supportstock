...
class Order extends Model
{
    ...
    public function place_order($request)
    {
        ...
        foreach ($cart_items as $key => $value) {
            if ($value->prodDiscount!='') {
                $tPrice = $value->final_price * $value->customers_basket_quantity;
                $disc = (($value->final_price*$value->prodDiscount)/100)*$value->customers_basket_quantity;
            ...
            if ($value->taxRate!='') {
              $taxVal = ($totalPrice*$value->taxRate)/100;
            }else{
              $taxVal = 0;
            }                            
        ...
        if (!empty($cust_address)) {
            ...
            $delivery_city = $cust_address[0]->cities_name;
            $delivery_pincode = $cust_address[0]->pincodes_val;
            $delivery_suburb = $cust_address[0]->flat_no;
        }
        ...
        $tax_rate = number_format((float) $totalTax, 2, '.', '');
        //$tax_rate = 0;
        $coupon_discount = number_format((float) session('coupon_discount'), 2, '.', '');
        $order_price = $request['products_price'];
        ...
        $total_tax = number_format((float) $totalTax, 2, '.', '');
        //$total_tax = 0;
        //$products_tax = 1;
        ...
        if (!empty(session('coupon')) and count(session('coupon')) > 0) {
            ...
            $cponArr = session('coupon');
            $coupon_text = $cponArr[0]->code;
        ...
        if ($payment_method == 'braintree') {
            ...
            $order_information = array(
                ...
                'IFSC' => $method['iban']->value,
                'Comment' => $method['swift']->value,
            );
        ...
        if ($payment_status == 'success') {            
            ...
            foreach ($cart_items as $products) {
                $products_tax = 0;
                if ($products->prodDiscount!='') {
                    $tPrice = $products->final_price * $products->customers_basket_quantity;
                    $disc = (($products->final_price*$products->prodDiscount)/100)*$products->customers_basket_quantity;
                    $totalPrice = $tPrice-$disc;
                }else{
                    $tPrice = $products->final_price * $products->customers_basket_quantity;
                    $disc = 0;
                ...
                if ($products->taxRate!='') {
                    $products_tax = ($totalPrice*$products->taxRate)/100;
                }
                ...
                $orders_products_id = DB::table('orders_products')->insertGetId(
                    [
                        ...
                        'bulk_discount' => $products->prodDiscount,
            ...
            $logoDetails = DB::table('invoice_logo')
                ...
                ->get();
            $ordersData['logoDetails'] = $logoDetails;
            ...
            $msg = "Your Order ID: ".$orders_id." Status: pending by Supportstock.";
        ...
        if (count($orders) > 0) {
            ...
            foreach ($orders as $orders_data) {
                ...
                $order_products = DB::table('orders_products')
                    ...
                    ->select('image_categories.path as image', 'products.products_model as model', 'orders_products.*', 'products.cancelation_period', 'products.return_period', 'products.products_status', 'products.products_sku','orders_status_reasons.status_reason')
                    ->where('orders_products.orders_id', $orders_data->orders_id)->groupBy('orders_products.orders_products_id')->get();
        ...
        if (count($orders) > 0) {
            ...
            foreach ($orders as $orders_data) {
                ...
                $order_products = DB::table('orders_products')
                    ...
                    ->LeftJoin('image_categories', 'products.products_image', '=', 'image_categories.image_id')
                    ->LeftJoin('orders_status_reasons', 'orders_status_reasons.reason_id', '=', 'orders_products.cancel_return_reason')
                    ->select('image_categories.path as image', 'products.products_model as model', 'products.products_status', 'products.products_sku', 'orders_products.*','orders_status_reasons.status_reason')
