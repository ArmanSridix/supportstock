...
class Customer extends Model
{
    ...
    public function handleSocialLoginCallback($social)
    {
        ...
        if ($social == 'facebook') {
            ...
            if (count($existUser) > 0 && $existUser[0]->role_id==2) {
            ...
            if (count($record) > 0) {
                ...
                if (!empty($request['customers_id'])) {
                    $total_wishlist = DB::table('liked_products')
                        ...
                        ->where('liked_customers_id', '=', $request['customers_id'])->count();
                }
                ...
                $responseData = array('success' => '2', 'message' => "Product is liked", 'total_likes' => $products[0]->products_liked, 'id' => 'like_count_' . $liked_products_id, 'total_wishlist' => $total_wishlist);
