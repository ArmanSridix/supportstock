...
class Products extends Model
{
    ...
    public function likedProducts()
    {
        ...
        $products = DB::table('liked_products')->where('liked_customers_id', '=', $customers_id)->get();
        $result = array();
        $index = 0;
...
///////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function products($data)
    {
        ...
        if (isset($data['today_deal'])) {
            if ($data['today_deal']!='') {
                $tdayDate = date('Y-m-d 00:00:00');
                $categories->where('products.today_deal', '=', '1');
                $categories->where('products.deal_start_date', '<=', $tdayDate);
                $categories->where('products.deal_end_date', '>=', $tdayDate);
            }            
        }

        if (isset($data['best_seller'])) {
            if ($data['best_seller']!='') {
                $categories->where('products.best_seller', '=', '1');
            }
        }

        if (isset($data['trending_product'])) {
            if ($data['trending_product']!='') {
                $categories->where('products.trending_product', '=', '1');
            }
        }
        ...
        if (count($products) > 0) {
            ...
            $index = 0;
            foreach ($products as $products_data) {
                $products_data->ss_assured_image = '';
                if ($products_data->ss_assured==1) {
                    $ssAssuredImage = DB::table('assured_logo')
                        ->LeftJoin('image_categories', 'assured_logo.assured_logo_image', '=', 'image_categories.image_id')
                        ->where('image_type', '=', 'ACTUAL')
                        ->first();

                    if ($ssAssuredImage) {
                        $products_data->ss_assured_image = $ssAssuredImage->path;
                    }
                }

                /*******************************************/
                $products_data->BulkPercentActive = 0;
                $products_data->BulkPercentMin = 0;
                $products_data->BulkPercentMax = 0;
                $BulkPrice = DB::table('product_bulk_price')
                    ->select('product_bulk_price_id', DB::raw('MIN(discount_rate) as BulkPercentMin'), DB::raw('MAX(discount_rate) as BulkPercentMax'))
                    ->where('products_id','=',$products_data->products_id);
                    
                    
                    // echo "<pre>";
                    // print_r($pro_category);
                    // exit;
                if(Session::has('supportUserDetails')){
                    ...
                    if ($current_user_details['user_type']=='Corporate') {
                        ...
                        if ($current_user_details['assign_type']=='Category') {
                           ...
                           // $productCategory = $detail['product_data'][0]->categories; // commented by barun
                           $productCategory = $this->getCategoriesByProductId($products_data->products_id);// added by barun
                            ...
                            $catCheck = 0;
                            if (in_array($products_data->products_id, $userProductArray)) {
                                $catCheck = 1;
                            }
                            if ($catCheck == 1) {
                                $BulkPrice->where('user_type','=','Corporate');
                ...
                if (count($BulkPriceList)>0) {
                    //echo "<pre>"; print_r($BulkPriceList); exit;
                    if ($BulkPriceList[0]->product_bulk_price_id != '' && $BulkPriceList[0]->BulkPercentMin != '' && $BulkPriceList[0]->BulkPercentMax != '') {
                        $products_data->BulkPercentActive = 1;
                        $products_data->BulkPercentMin = $BulkPriceList[0]->BulkPercentMin;
                        $products_data->BulkPercentMax = $BulkPriceList[0]->BulkPercentMax;
                    }
                }
                /*******************************************/
                        ...
                        $join->on('image_categories.image_id', '=', 'products_images.image')
                            ->where(function ($query) {
                                $query->where('image_categories.image_type', '=', 'LARGE')
                                    ->where('image_categories.image_type', '!=', 'LARGE')
                ...
                if (Session::has('supportUserDetails')) {
                    $current_user_details = Session::get('supportUserDetails');
                    $liked_customers_id = $current_user_details['id'];
    ...
    public function cartIdArray($requestt)
    {
        ...
        if (Session::has('supportUserDetails')) {
            ...
            $cart->where('customers_basket.customers_id', '=', '');
        }
    ...
    public function productQuantity($data)
    {
        if (!empty($data['attributes'])) {
            ...
            if (!empty($inventory_ref_id) && isset($inventory_ref_id[0])) {
               ...
               $minMax =  $inventories = DB::table('products')->where('products_id', $products_id)->get();
            } else {
                $minMax = '';
            }
