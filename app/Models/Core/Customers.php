...
class Customers extends Model
{
    ...
    public function getter(){
        ...
        $user = User::sortable(['id'=>'ASC'])
            ...
            ->LeftJoin('cities','cities.cities_id','=', 'address_book.entry_city')
            ->LeftJoin('pincodes','pincodes.pincodes_id','=', 'address_book.entry_postcode')
            ...
            'address_book.entry_state as entry_state', 'countries.*', 'zones.*','cities.cities_name','pincodes.pincodes_val')
            ->where('role_id',2)
            ->groupby('users.id')
            ->get();
    ...
    public static function getCorporateRequest(){
    ...
    public function emailEdit($request){
        $existEmail = DB::table('users')->where('email', '=', $request->email)->where('id', '!=', $request->customers_id)->get();
        return $existEmail;
    }

    public function phoneEdit($request){
        $existPhone = DB::table('users')->where('phone', '=', $request->phone)->where('id', '!=', $request->customers_id)->get();
        return $existPhone;
    }
    ...
    public function insert($request){
      ...
      if ($request->user_type=='Corporate') {
        if ($request->subscription_start_date=='' && $request->subscription_end_date=='') {
          ...
          $subscription_start = date('Y-m-d H:i:s',strtotime($subscription_start_date));
          $subscription_end = date('Y-m-d H:i:s',strtotime($subscription_end_date));
        }
      }else{
        $subscription_period = null;
        $subscription_start = null;
    ...
    public function destroyrecord($user_id){
      ...
      DB::table('user_to_category')->where('user_id','=', $user_id)->delete();
    }
        ...
        switch ( $filter )
        {
            case 'Name':
                $user = DB::table('users')
                    ...
                    'address_book.entry_street_address as entry_street_address', 'address_book.entry_suburb as entry_suburb',
                    'address_book.entry_postcode as entry_postcode', 'address_book.entry_city as entry_city',
                    'address_book.entry_state as entry_state', 'countries.*', 'zones.*','image_categories.path as path','cities.cities_name','pincodes.pincodes_val')
            ...
            case 'Status':
            $parameter = $request->parameter == 'active' ? 1 : 0;
            // dd($parameter);
            $user = DB::table('users')
                ->LeftJoin('user_to_address', 'user_to_address.user_id' ,'=', 'users.id')
                ->LeftJoin('address_book','address_book.address_book_id','=', 'user_to_address.address_book_id')
