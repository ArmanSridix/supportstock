...
class Products extends Model
{
	...
	public function paginator($request){
        ...
        if (isset($_REQUEST['categories_id']) and !empty($_REQUEST['categories_id'])) {
            ...
            if($_REQUEST['categories_id'] == 'sku'){
                if (isset($_REQUEST['product']) and !empty($_REQUEST['product'])) {
                    $data->where('products.products_sku', '=', $_REQUEST['product'] );
                }
            }else if($_REQUEST['categories_id'] == 'Active'){
                $data->where('products.products_status', '=', 1 );
            }else if($_REQUEST['categories_id'] == 'Inactive'){
                $data->where('products.products_status', '=', 0 );
            }else{
  ...
  public function insert($request){
    ...
    if ($request->trending_product){
        $trending_product = $request->trending_product;
    }else{
        $trending_product = 0;
    }
    if ($request->ss_assured){
        $ss_assured = $request->ss_assured;
    }else{
        $ss_assured = 0;
    }

    if ($request->deal_start_date!='') {
      $dealDate1 = str_replace('/', '-', $request->deal_start_date);
      $deal_start_date = date('Y-m-d H:i:s', strtotime($dealDate1));
    }else{
      $deal_start_date = null;
    }

    if ($request->deal_end_date!='') {
      $dealDate2 = str_replace('/', '-', $request->deal_end_date);
      $deal_end_date = date('Y-m-d H:i:s', strtotime($dealDate2));
    }else{
      $deal_end_date = null;
    }
    ...
    $products_id = DB::table('products')->insertGetId([
        ...
        'corporate_products_price' => $request->corporate_products_price,
        ...
        'ss_assured' => $ss_assured,
        'cancelation_period' => $request->cancelation_period,
        'return_period' => $request->return_period
    ]);

    if ($products_id) {
      if (!empty($request->discount_rate)) {
        for($i=0;$i<count($request->discount_rate);$i++){
          $product_bulk_price_id = DB::table('product_bulk_price')->insertGetId([
            'products_id' => $products_id,
            'user_type' => 'Normal',
            'minimum_quantity' => $request->minimum_quantity[$i],
            'maximum_quantity' => $request->maximum_quantity[$i],
            //'bulk_amount' => $request->bulk_amount[$i],
            'discount_rate' => $request->discount_rate[$i]
          ]);    
        }          
      }

      if (!empty($request->corp_discount_rate)) {
        for($i=0;$i<count($request->corp_discount_rate);$i++){
          $product_bulk_price_id = DB::table('product_bulk_price')->insertGetId([
            'products_id' => $products_id,
            'user_type' => 'Corporate',
            'minimum_quantity' => $request->corp_minimum_quantity[$i],
            'maximum_quantity' => $request->corp_maximum_quantity[$i],
            //'bulk_amount' => $request->corp_bulk_amount[$i],
            'discount_rate' => $request->corp_discount_rate[$i]
          ]);    
        }          
      }
    }
    ...
    foreach($languages as $languages_data){
        ...
        $pro_quick_overview = 'pro_quick_overview_'.$languages_data->languages_id;
        //left banner
        ...
        $req_pro_quick_overview = $request->$pro_quick_overview;
        DB::table('products_description')->insert([
            ...
            'pro_quick_overview' => addslashes($req_pro_quick_overview)
        ]);
    }
    ...
    foreach($result['languages'] as $languages_data){
      $description = DB::table('products_description')
        ...
        if(count($description)>0){
            ...
            $description_data[$languages_data->languages_id]['pro_quick_overview'] = $description[0]->pro_quick_overview;
            ...
            $description_data[$languages_data->languages_id]['pro_quick_overview'] = '';
  ...
  public function updaterecord($request){
          ...
          DB::table('product_bulk_price')->where([
            'products_id' => $products_id,
          ])->delete();
  ...
  public function deleterecord($request){
    ...
    $categories = DB::table('product_bulk_price')->where('products_id', $products_id)->delete();
  ...
  public function addinventoryfromsidebar(){
    ...
    if(count($product)>0){
        ...
        // dd($result['products'][0]->products_type);
    if($result['products'][0]->products_type!=1){
    ...
    $products_attribute = DB::table('products_attributes')->where('products_id', $result['products'][0]->products_id)->get();
    $products_attribute = $products_attribute->unique('options_id')->keyBy('options_id');
    ...
    $product = DB::table('products')
        ...
        if($products[0]->products_type==1){
            ...
            DB::table('inventory_detail_new')->insert([
                'inventory_ref_id' => $inventory_ref_id,
                'products_id' => $products_id,
                'attributes_ids' => $attribute_string,
            ]);
        }
        return true;
  }
...
//         print_r('(select count(*) from `inventory_detail` where `inventory_detail`.`attribute_id` in (' . $attributeid . ') and `inventory_ref_id`= "' . $inventory->inventory_ref_id . '")');
        

// print_r($inventory);
            $individualStock = DB::table('inventory')->leftjoin('inventory_detail', 'inventory_detail.inventory_ref_id', '=', 'inventory.inventory_ref_id')
            ->selectRaw('inventory.*')
            ->whereIn('inventory_detail.attribute_id', explode(',',$attributeid))



            ->where(DB::raw('(select count(*) from `inventory_detail` where `inventory_detail`.`attribute_id` in (' . $attributeid . ') and `inventory_ref_id`= "' . $inventory->inventory_ref_id . '")'), '=',5)
